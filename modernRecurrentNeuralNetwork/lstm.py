...
# 引入记忆元（memory cell），输出门（output gate）,输入门（input gate），遗忘门（forget gate）
...
def lstm_scratch(inputs, state, params):
    """长短期记忆网络"""
...
def test_lstm_scratch():
    ...
    num_epochs, lr = 500, 1
    model = RNNModelScratch(len(vocab), num_hiddens, device, get_lstm_params, init_lstm_state, lstm_scratch)
