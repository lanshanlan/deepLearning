...
from utils.train_ch3 import Accumulator, Animator
from utils.activatefunc import sgd
...
def test_one_hot():
    lenVocab = len(vocab)
    one_hot_vocab = F.one_hot(torch.tensor([0, 2]), lenVocab)
    print('lenVocab\n', lenVocab)
    print('one_hot_vocab\n', one_hot_vocab.shape)
...
def test_predict_ch8():
    num_hiddens = 512
    net = RNNModelScratch(len(vocab), num_hiddens, try_gpu(), get_params,
                          init_rnn_state, rnn)
    pre = predict_ch8('time traveller ', 10, net, vocab, try_gpu())
    print(pre)
    ...
    for X, Y in train_iter:
        if state is None or use_random_iter:
            ...
            if isinstance(net, nn.Module) and not isinstance(state, tuple):
                ...
                # state对于LSTM或对于我们从零开始实现的模型是一个由张量组成的元组
                for s in state:
                    s.detach_()
        y = Y.T.reshape(-1)
